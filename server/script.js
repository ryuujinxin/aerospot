...
function handleFormSubmit(e) {
    ...
    if (inputMode === 'url') {
        ...
        saveSpotData(imageUrl, airline, aircraftType, registration, category, location, cameraGear, spotDate, notes);
    } else {
        ...
        reader.onload = function(uploadEvent) {
            ...
            saveSpotData(tempImageBase64, airline, aircraftType, registration, category, location, cameraGear, spotDate, notes);
        };
        reader.readAsDataURL(file);
    }
}

function saveSpotData(imgSrc, airline, aircraftType, registration, category, location, cameraGear, spotDate, notes) {
    const newSpot = {
        id: Date.now(),
        image: imgSrc,
        airline,
    ...
    grid.innerHTML = filtered.map(item => `
        <div class="bg-white/10 backdrop-blur-md border border-white/15 rounded-3xl overflow-hidden shadow-xl hover:border-aviation-sky/50 transition-all duration-300 flex flex-col group">
            <div class="relative h-52 overflow-hidden bg-black/40 cursor-pointer" onclick="openDetail(${item.id})">
                ...
                <div class="absolute top-3 right-3 bg-black/60 backdrop-blur-md px-3 py-1 rounded-full text-xs font-semibold text-aviation-sky border border-white/10 shadow-md">
                    ${item.category}
                </div>
...
function setCategoryFilter(category) {
    ...
    buttons.forEach(btn => {
        if (btn.textContent.toLowerCase().includes(category.toLowerCase()) || (category === 'all' && btn.textContent.includes('Semua'))) {
...
function openDetail(id) {
    ...
    document.getElementById('detailContent').innerHTML = `
        ...
        <div class="grid grid-cols-2 gap-4 pt-2">
            ...
            <div class="bg-white/5 p-3 rounded-xl border border-white/10">
                <span class="text-xs text-slate-400 block">Lokasi / Bandara</span>
